...
# запуск тестов: python -m pytest .\movielens_analysis.py -v
# -v для подробного вывода, -m запускать определенный модуль как скрипт
...
class Ratings:
    ...
    def __init__(self, path_to_the_file):
        self.path = path_to_the_file    # указываем при создании объекта
        self.ratings_data = self.load_ratings_data()     # загрузка оценок
        self.movies_data = self.load_movies_data()       # загрузка названий
        self.movies = self.Movies(self)       # объект подкласса
        self.users = self.Users(self)        # тоже
    ...
    def load_ratings_data(self):
        ...
        with open(self.path, 'r', encoding='utf-8') as f:
            headers = f.readline().strip().split(',')     # readline читает первую строку
            for i, line in enumerate(f):
                if i >= 1000: break
                values = line.strip().split(',')
                row = dict(zip(headers, values))    # zip объединяет два списка попарно (Создает пары, типо 'userId': '1')

                row['userId'] = int(row['userId'])   # конвертирует '1' в 1 в строке userId
    ...
    def load_movies_data(self):
        ...
        with open (movies_path, 'r', encoding='utf-8') as f:
            f.readline()   # пропуск заголовков
            for line in f:
                line = line.strip()
                first_comma = line.find(',')    # позиция первой запятой 
                last_comma = line.rfind(',')    # например 35 (считаем по символам)

                movie_id = int(line[:first_comma])
                title = line[first_comma + 1: last_comma]

                if title.startswith('"') and title.endswith('"'):
                    title = title[1:-1]    # убираем кавычки
    ...
    class Movies:    
        ...
        def __init__(self, rating_obj):      # объект Rating
            self.rating_obj = rating_obj      # кладем ссылку на него в self.rating_obj, то бишь создаем свой rating_obj


        # в каком году сколько оценок
        def dist_by_year(self):

            year_counts = defaultdict(int)                               # создание умного словаря (делает ключи автоматически)


            for row in self.rating_obj.ratings_data:
                timestamp = row['timestamp']
                year = datetime.fromtimestamp(timestamp).year     # делаем норм дату
                year_counts[year] += 1      # ищем запись для [year], если нет создаем новую запсиь (Defaultdict)

            ratings_by_year = dict(sorted(year_counts.items()))

            return ratings_by_year   #items делает из словаря список пар, потом сортировка, потом снова в словарь
        


        # сколько раз поставили каждую оценку
        ...
        # топ по кол-ву оценок (не важно каких!)
        def top_by_num_of_ratings(self, n):

            movie_ratings_count = defaultdict(int)        # вытаскиваем айдишник и кол-во отзывов на него (одна строка в ratings = одна оценка)
            ...
            movies_with_titles = {}                              # объединение данных (без умного словаря. мы напрямую вытаскиваем названия по айди)
            ...
            top_movies = dict(sorted(
                movies_with_titles.items(),   # делаем список кортежей
                key = lambda x: x[1],    # сортируем по значениям, а не по ключам
                reverse = True
            ) [:n])                          # берем первые n элементов
        ...
        def top_by_ratings(self, n, metric='mean'):
            ...
            if metric == 'mean':
                ...
                calc = lambda x: sum(x) / len(x)  # по умолчанию mean
            
            movie_ratings = defaultdict(list)

            # все оценки для каждого фильма
            for row in self.rating_obj.ratings_data:
                movie_id = row['movieId']
                rating = row['rating']
            ...
            # средняя оценка для кажого фильма
            movie_scores = {}
            ...
            # объединям названия с айдишниками
            movies_with_titles = {}
        ...
        # чаще всего смотрят и оценивают в выходные
        def weekend_hits(self, n):
            ...
            for row in self.rating_obj.ratings_data:
                ...
                weekday = datetime.fromtimestamp(timestamp).weekday()  # 5 - суббота
                if weekday >= 5:
        ...
        def top_controversial(self, n):
            ...
            movie_ratings = defaultdict(list)   # указываем ЧТО создавать для новых ключей (список!)
            ...
            # дисперсия (разброс оценок)
            movie_variances = {}
            for movie_id, ratings in movie_ratings.items():
                if len(ratings) > 1:    # дисперсия считается от 2 оценок
                    ...
                    # одна оценка = дисперсия 0
                    movie_variances[movie_id] = 0.0


            """
            считаем среднюю оценку. после из каждой оценки вычитаем среднюю оценку.
            эту разницу возводим в квадрат. складываем все квадраты разниц и делим сумму
            на кол-во оценок
            """
            ...
            # сортировка по дисперсии по убыванию
            top_movies = dict(sorted(
                movies_with_titles.items(),
                key=lambda x: x[1],
                reverse = True
    ...
    class Users(Movies):
        ...
        def dist_by_num_ratings(self):
            ...
            ratings_by_users = dict(sorted(     # чисто для удобства
        ...
        def night_monsters(self, n):
            ...
            # процент ночных оценок для каждого
            night_ratings = {}
            for user_id in user_all_ratings:
                if user_all_ratings[user_id] >= 5:
                    ratings = user_night_ratings.get(user_id, 0) / user_all_ratings[user_id]   # делим ночные на общие
        ...
        # самые спорные ребята (от любви до ненависти)
...
class Tags:
 ...
 Drop the duplicates. Sort it by numbers descendingly.
    ...
    def most_popular_by_movie(self, movie_id, n):
        """
        Возвращает топ-n самых популярных тегов для конкретного фильма.
        Это словарь: {тег: количество}.
        """
        tag_counts = {}
    ...
    def average_tag_length(self):
        """
        Возвращает среднюю длину тега (в символах).
        """
    ...
    def get_unique_tags_count(self):
        """
        Возвращает общее количество уникальных тегов.
        """
        unique_tags = set()
        for _, _, tag, _ in self.tags_data:
            unique_tags.add(tag)
...
class Tests:
    ...
    def test_ratings_initializations(self):
        # чекаем инициализацию Ratings
        rating_obj = Ratings('data/ml-latest-small/ratings.csv')
        assert rating_obj is not None
        assert hasattr(rating_obj, 'movies')    # проверяет наличие атрибута (объект класса Movies)
    ...
    def test_dist_by_year(self):
        ...
        assert isinstance(result, dict)   # проверяет тип данных (хочет видеть словарь)

        # проверяет элементы словаря
        for year, count in result.items():    # чекаем типы данных
            assert isinstance(year, int)
            assert isinstance(count, int)

        # проверка на сортировку
        years = list(result.keys())    # получает список годов 
    ...
    def test_top_by_ratings_mean(self):
        ...
        if result:    # защита от пустого словаря (если у фильмов нет достаточно оценок)
            ratings = list(result.values())
            assert ratings == sorted(ratings, reverse=True)


    # то же самое но с median
    ...
    def test_weekend_hits(self):
        ...
        for title, ratio in result.items():
            ...
            assert 0 <= ratio <= 1  # процент должен быть от 0 до 1
